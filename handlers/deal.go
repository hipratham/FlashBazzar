...
func NewDealHandler(firestoreClient *firestore.Client) *DealHandler {
	return &DealHandler{
...
// GetTodayDealHandler finds the currently active deal and returns it with countdown data.
func (h *DealHandler) GetTodayDealHandler(c *gin.Context) {
	now := time.Now()

	// Query for deals where StartTime is in the past and EndTime is in the future
	query := h.FirestoreService.Client.Collection("deals").
		Where("StartTime", "<=", now).
		Where("EndTime", ">=", now).
		Limit(1) // Assuming only one active deal at a time

	docs, err := query.Documents(c).GetAll()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch today's deal", "details": err.Error()})
		return
	}

	if len(docs) == 0 {
		c.JSON(http.StatusNotFound, gin.H{"message": "No active deal found for today"})
		return
	}

	// TODO: Implement update deal logic - will be added in a separate step
}

// DeleteDealHandler handles deleting a deal.
// Requires vendor authentication and ownership of the deal.
func (h *DealHandler) DeleteDealHandler(c *gin.Context) {
	// TODO: Implement delete deal logic - will be added in a separate step
}
}
package handlers
